...
return <div className="app"><header><div className="header-brand"><img src="/CapgeminiEngineering_Logo_2COL_RGB.svg" alt="Capgemini Engineering" className="brand-logo"/><b>Velocity AI</b><span>Requirements Generator</span></div><div className="header-right"><span className={`llm-badge ${llmCfg.has_key?'llm-on':'llm-off'}`}>{llmCfg.has_key?'● LLM':'○ Demo'}</span><button className="settings-btn" onClick={()=>setShowSettings(v=>!v)} title="LLM Settings">⚙</button><div className="status">{busy?'● ':''}{msg}</div></div></header>
